...
Type
  ...
  actionf_t = Record
    Case byte Of
      ...
      1: (acp1: actionf_p1);
      2: (acp2: actionf_p2);
      3: (acp3: actionf_p3); // [crispy] let pspr action pointers get called from mobj states
  End;
  ...
  subsector_t = Record
    ...
    sector: Psector_t;
    numlines: int; // [crispy] extended nodes
    firstline: int; // [crispy] extended nodes
  End;
  ...
  mobj_t = Record
    ...
    frame: int; // might be ORed with FF_FULLBRIGHT
    ...
    movedir: int; // 0-7
    movecount: int; // when 0, select a new dir
  ...
  sector_t = Record
    ...
    linecount: int;
    lines: Array Of line_t; // [linecount] size
    ...
    oldfloorheight: fixed_t;
    oldceilingheight: fixed_t;
    ...
    //      the renderer.
    interpfloorheight: fixed_t;
    interpceilingheight: fixed_t;
  ...
  player_t = Record
    ...
    powers: Array[0..integer(NUMPOWERS) + 3 - 1] Of int; // [crispy] showfps and mapcoords are now "powers"
    cards: Array[card_t] Of boolean;
    backpack: boolean;
    ...
    psprites: Array[psprnum_t] Of pspdef_t;
    ...
    lookdir, oldlookdir: int;
    centering: boolean;

    // [crispy] jumping
    jumpTics: unsigned_int;

    // [crispy] weapon recoil pitch
    recoilpitch, oldrecoilpitch: fixed_t;
